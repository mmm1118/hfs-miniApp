    ...
    myapi.dynamic(sessionId, dynamicid).then(res=>{
      ...
      if (res.data.status==200){ 
        ...
        let baselist = res.data.data.bussData;
          ...
          var timeatc = utiltwo.funtime(baselist.createtime);
          if (timeatc.dw == '分') {
            baselist.createtime = timeatc.time + '分钟前';

          } else if (timeatc.dw == '时') {
            baselist.createtime = timeatc.time + '小时前';
          } else {
            const timespliceLength = baselist.createtime.split(' ')
            const hf = timespliceLength[1].split(':')
            baselist.createtime = utiltwo.Annualconversion(timespliceLength[0])+ hf[0] + ':' + hf[1];
          }
...
/* 获取动态列表 */
  ...
  onShareAppMessage: function(){
    /*坑点 微信分享成功或者失败无法得到结果  官网给出的回复*/
    const that=this;
    const sessionId = wx.getStorageSync('sessionId');
    console.log(that.data.bussData.location)
      return {
        title: `${that.data.bussData.location}帖子详情`, //转发的标题,
